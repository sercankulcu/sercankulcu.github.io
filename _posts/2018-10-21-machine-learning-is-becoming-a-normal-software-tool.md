...
Machine learning is no longer something I hear about only in research papers. TensorFlow was open-sourced in November 2015. Keras made it more approachable — and Google officially adopted Keras as TensorFlow's high-level API in 2017. PyTorch, released by Facebook's research lab in September 2016, gained significant traction in academic research through 2017 and 2018 because its dynamic computation graph made debugging more natural. scikit-learn has made classical machine-learning algorithms available in Python with a consistent API since 2011. Cloud platforms — AWS SageMaker (launched November 2017), Google Cloud ML Engine, Azure Machine Learning — now offer managed environments for training and deploying models without running your own infrastructure.

2018 has been a significant year for language models. OpenAI published GPT-1 in June, demonstrating that a large transformer model pretrained on text could be fine-tuned for downstream tasks with much less labelled data. Google published BERT in October 2018 — two weeks before this post — showing that bidirectional pretraining produced language representations that improved the state of the art on eleven NLP benchmarks simultaneously. Both papers suggest that the pattern of large-scale pretraining followed by task-specific fine-tuning will become the dominant approach in natural language processing.
