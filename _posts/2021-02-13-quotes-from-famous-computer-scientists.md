...
Edsger Dijkstra often reminded programmers that simplicity and clarity require discipline. His writing can sound severe, but the message is important: programs are read by people as well as executed by machines.

Donald Knuth's famous warning that "premature optimization is the root of all evil" is also useful when understood in context. Performance matters, but optimizing before we understand the real problem can make software complicated for no good reason.
